...
export const getInventoryByMenuItemId = async (menuItemId: string, storeId: string) => {
    return db.query.inventory.findFirst({
        where: and(eq(inventory.menuItemId, menuItemId), eq(inventory.storeId, storeId)),
        with: { menuItem: { columns: { name: true, itemCode: true } } },
    });
};

export const calculateInventoryStatus = (quantity: number, minStockLevel: number): StockStatus => {
 ...
 */
export const calculateMenuItemCost = async (menuItemId: string, storeId: string) => {
    const ingredients = await db
        ...
        .where(and(eq(billOfMaterials.menuItemId, menuItemId), eq(billOfMaterials.storeId, storeId)));
