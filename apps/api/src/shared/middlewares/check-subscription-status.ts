...
export const checkSubscriptionStatus = async (req: CustomRequest, res: Response, next: NextFunction) => {
    const currentUser = req.user?.data;
    const storeId = currentUser?.storeId;

    if (!storeId) {
        // This should ideally be caught by `checkUserHasStore` middleware first
        return handleError2(res, "User not authenticated.", StatusCodes.UNAUTHORIZED);
    }
