 ...
 */
export const getUserStoreScope = async (role: UserRoleEnum, storeId: string): Promise<string[] | null> => {
    // Admin, User, Guest only sees their assigned store.
    if (role === UserRoleEnum.ADMIN || role === UserRoleEnum.USER || role === UserRoleEnum.GUEST) {
        return [storeId];
    }
    ...
    if (role === UserRoleEnum.MANAGER) {
        ...
        const branchStores = await db.select({ id: stores.id }).from(stores).where(eq(stores.storeParentId, storeId));
